...
public class Source implements InsertSourceHandler, IsWidget,
		OpenSourceFileHandler, TabClosingHandler, TabCloseHandler,
		TabReorderHandler, SelectionHandler<Integer>, TabClosedHandler,
		FileEditHandler, ShowContentHandler, ShowDataHandler,
		CodeBrowserNavigationHandler, CodeBrowserFinishedHandler,
		CodeBrowserHighlightEvent.Handler,
		SourceExtendedTypeDetectedEvent.Handler, BeforeShowHandler {
	public interface Display extends IsWidget, HasTabClosingHandlers,
			HasTabCloseHandlers, HasTabClosedHandlers, HasTabReorderHandlers,
			HasBeforeSelectionHandlers<Integer>, HasSelectionHandlers<Integer> {
		void addTab(Widget widget, ImageResource icon, String name,
				String tooltip, boolean switchToTab);
		...
		void showUnsavedChangesDialog(String title,
		...
		void renameTab(Widget child, ImageResource icon, String value,
				String tooltip);
	...
	@Inject
	public Source(Commands commands, Display view,
			SourceServerOperations server,
			EditingTargetSource editingTargetSource,
			FileTypeRegistry fileTypeRegistry, GlobalDisplay globalDisplay,
			FileDialogs fileDialogs, RemoteFileSystemContext fileContext,
			EventBus events, final Session session, Synctex synctex,
			WorkbenchContext workbenchContext, Provider<FileMRUList> pMruList,
			UIPrefs uiPrefs, RnwWeaveRegistry rnwWeaveRegistry) {
		commands_ = commands;
		view_ = view;
		server_ = server;
		...
		commands.goToFunctionDefinition()
				.setShortcut(new KeyboardShortcut(113));
		...
		if (BrowseCap.isMacintosh()) {
			ShortcutManager.INSTANCE.register(KeyboardShortcut.META
					| KeyboardShortcut.ALT, 192, commands.executeNextChunk(),
					"Execute", commands.executeNextChunk().getMenuLabel(false),
					"");
		}
		...
		events.addHandler(ViewDataEvent.TYPE, new ViewDataHandler() {
			public void onViewData(ViewDataEvent event) {
				server_.newDocument(FileTypeRegistry.DATAFRAME.getTypeId(),
						null, JsObject.createJsObject(),
		...
		events.addHandler(FileTypeChangedEvent.TYPE,
				new FileTypeChangedHandler() {
		...
		events.addHandler(SourceOnSaveChangedEvent.TYPE,
				new SourceOnSaveChangedHandler() {
					@Override
					public void onSourceOnSaveChanged(
							SourceOnSaveChangedEvent event) {
						manageSaveCommands();
					}
				});
		...
		events.addHandler(SourceFileSavedEvent.TYPE,
		...
		events.addHandler(SynctexStatusChangedEvent.TYPE,
				new SynctexStatusChangedEvent.Handler() {
					@Override
					public void onSynctexStatusChanged(
							SynctexStatusChangedEvent event) {
		...
		new IntStateValue(MODULE_SOURCE, KEY_ACTIVETAB,
				ClientState.PROJECT_PERSISTENT, session.getSessionInfo()
						.getClientState()) {
			@Override
		...
		uiPrefs_.verticallyAlignArgumentIndent().bind(
		...
		uiPrefs_.useVimMode().bind(new CommandWithArg<Boolean>() {
			...
			public void execute(Boolean arg) {
				ShortcutManager.INSTANCE
						.setEditorMode(arg ? KeyboardShortcut.MODE_VIM
	...
	private void initVimCommands() {
		...
		vimCommands_.showVimHelp(RStudioGinjector.INSTANCE.getShortcutViewer());
	 ...
	 *            True if a new tab is about to be created. (If false and there
	 *            are no tabs already, then a new source doc might be created to
	 *            make sure we don't end up with a source pane showing with no
	 *            tabs in it.)
	 */
	...
	private void restoreDocuments(final Session session) {
		final JsArray<SourceDocument> docs = session.getSessionInfo()
				.getSourceDocuments();
	...
	public void onShowContent(ShowContentEvent event) {
		...
		server_.newDocument(FileTypeRegistry.URLCONTENT.getTypeId(), null,
	...
	public void onShowData(ShowDataEvent event) {
		...
		ensureVisible(true);
		server_.newDocument(FileTypeRegistry.DATAFRAME.getTypeId(), null,
				(JsObject) data.cast(),
	...
	public void onShowProfiler() {
		...
		// create new profiler
		ensureVisible(true);
		server_.newDocument(FileTypeRegistry.PROFILER.getTypeId(), null,
	...
	public void onNewCppDoc() {
		if (uiPrefs_.useRcppTemplate().getValue()) {
			newSourceDocWithTemplate(FileTypeRegistry.CPP, "", "rcpp.cpp",
					Position.create(0, 0), new CommandWithArg<EditingTarget>() {
						@Override
	...
	public void onNewSweaveDoc() {
		...
		if (uiPrefs_.alwaysEnableRnwConcordance().getValue()) {
			RnwWeave activeWeave = rnwWeaveRegistry_
					.findTypeIgnoreCase(uiPrefs_.defaultSweaveEngine()
							.getValue());
		...
		// get the template
		server_.getSourceTemplate("", "sweave.Rnw",
				new ServerRequestCallback<String>() {
					...
					public void onResponseReceived(String templateContents) {
						...
						final boolean hasConcordance = concordanceValue
								.length() > 0;
						if (hasConcordance) {
							String beginDoc = "\\begin{document}\n";
							templateContents = templateContents.replace(
									beginDoc, beginDoc + concordanceValue);
						}
						...
						newDoc(FileTypeRegistry.SWEAVE,
								...
								new ResultCallback<EditingTarget, ServerError>() {
									@Override
									public void onSuccess(EditingTarget target) {
										int startRow = 4 + (hasConcordance ? 1
												: 0);
	...
	public void onNewRDocumentationDoc() {
		new NewRdDialog(new OperationWithInput<NewRdDialog.Result>() {
				...
				final Command createEmptyDoc = new Command() {
					...
					public void execute() {
						newSourceDocWithTemplate(FileTypeRegistry.RD,
								result.name, "r_documentation_empty.Rd",
								Position.create(3, 7));
					}
				};

				if (!result.type.equals(NewRdDialog.Result.TYPE_NONE)) {
					server_.createRdShell(result.name, result.type,
							new SimpleRequestCallback<RdShellResult>() {
								@Override
								public void onResponseReceived(
										RdShellResult result) {
									if (result.getPath() != null) {
										fileTypeRegistry_
												.openFile(FileSystemItem
														.createFile(result
																.getPath()));
	...
	public void onNewRPresentationDoc() {
		fileDialogs_.saveFile("New R Presentation", fileContext_,
				workbenchContext_.getDefaultFileDialogDir(), ".Rpres", true,
				new ProgressOperationWithInput<FileSystemItem>() {
					...
					public void execute(final FileSystemItem input,
						...
						server_.createNewPresentation(input.getPath(),
	...
	private void newRMarkdownV1Doc() {
		newSourceDocWithTemplate(FileTypeRegistry.RMARKDOWN, "",
				"r_markdown.Rmd", Position.create(3, 0));
	}

	private void newRMarkdownV2Doc() {
		rmarkdown_.withRMarkdownPackage("Creating R Markdown documents", false,
					...
					public void execute(RMarkdownContext context) {
						new NewRMarkdownDialog(
								...
								new OperationWithInput<NewRMarkdownDialog.Result>() {
									@Override
									public void execute(
											final NewRMarkdownDialog.Result result) {
										if (result.isNewDocument()) {
											NewRMarkdownDialog.RmdNewDocument doc = result
													.getNewDocument();
											String author = doc.getAuthor();
											if (author.length() > 0) {
												uiPrefs_.documentAuthor()
														.setGlobalValue(author);
	...
	private void newRMarkdownV2Doc(final NewRMarkdownDialog.RmdNewDocument doc) {
		rmarkdown_.frontMatterToYAML(
				(RmdFrontMatter) doc.getJSOResult().cast(), null,
				new CommandWithArg<String>() {
					@Override
					public void execute(final String yaml) {
						String template = "";
						// select a template appropriate to the document type
						// we're creating
						...
						newSourceDocWithTemplate(FileTypeRegistry.RMARKDOWN,
								"", template, Position.create(1, 0), null,
	...
	private void newSourceDocWithTemplate(final TextFileType fileType,
			String name, String template, final Position cursorPosition) {
	...
	private void newSourceDocWithTemplate(final TextFileType fileType,
		...
		newSourceDocWithTemplate(fileType, name, template, cursorPosition,
				onSuccess, null);
	}
	...
	private void newSourceDocWithTemplate(final TextFileType fileType,
		...
		server_.getSourceTemplate(name, template,
				new ServerRequestCallback<String>() {
					@Override
					public void onResponseReceived(String templateContents) {
						indicator.onCompleted();
						...
						if (contentTransformer != null)
							templateContents = contentTransformer
									.transform(templateContents);
	...
	private void newDoc(EditableFileType fileType, final String contents,
			final ResultCallback<EditingTarget, ServerError> resultCallback) {
		ensureVisible(true);
		server_.newDocument(fileType.getTypeId(), contents, JsObject
				.createJsObject(), new SimpleRequestCallback<SourceDocument>(
	...
	private void saveEditingTargetsWithPrompt(String title,
			ArrayList<EditingTarget> editingTargets, final Command onCompleted,
		...
		// otherwise use the multi save changes dialog
			...
			ArrayList<UnsavedChangesTarget> unsavedTargets = new ArrayList<UnsavedChangesTarget>();
			unsavedTargets.addAll(editingTargets);

			// show dialog
			view_.showUnsavedChangesDialog(title, unsavedTargets,
	...
	public void closeAllSourceDocs(String caption, Command onCompleted) {
		...
		// save targets
		saveEditingTargetsWithPrompt(caption, dirtyTargets,
				CommandUtil.join(closeAllTabsCommand, onCompleted), null);
	...
	public void saveWithPrompt(UnsavedChangesTarget target,
			Command onCompleted, Command onCancelled) {
				...
				new CPSEditingTargetCommand() {
					...
					public void execute(EditingTarget saveTarget,
						...
						if (saveTarget.getPath() != null) {
							...
							// untitled document -- just close the tab
							// non-interactively
							view_.closeTab(saveTarget.asWidget(), false,
									continuation);
						}
					}
				},
	...
	public void onOpenSourceDoc() {
		fileDialogs_.openFile("Open File", fileContext_,
				...
				new ProgressOperationWithInput<FileSystemItem>() {
					public void execute(final FileSystemItem input,
						...
						workbenchContext_.setDefaultFileDialogDir(input
								.getParentPath());
	...
	public void onOpenSourceFile(OpenSourceFileEvent event) {
		doOpenSourceFile(event.getFile(), event.getFileType(),
				event.getPosition(), null, event.getNavigationMethod(), false);
	}

	public void onOpenPresentationSourceFile(
			OpenPresentationSourceFileEvent event) {
		// don't do the navigation if the active document is a source
		// file from this presentation module

		doOpenSourceFile(event.getFile(), event.getFileType(),
				event.getPosition(), event.getPattern(),
				NavigationMethod.HighlightLine, true);

	}

	public void onEditPresentationSource(final EditPresentationSourceEvent event) {
		openFile(event.getSourceFile(), FileTypeRegistry.RPRESENTATION,
				new CommandWithArg<EditingTarget>() {
					@Override
					public void execute(final EditingTarget editor) {
						TextEditingTargetPresentationHelper.navigateToSlide(
								editor, event.getSlideIndex());
					}
				});
	}

	private void doOpenSourceFile(final FileSystemItem file,
			final TextFileType fileType, final FilePosition position,
			final String pattern, final NavigationMethod navMethod,
			final boolean forceHighlightMode) {
		final boolean isDebugNavigation = navMethod == NavigationMethod.DebugStep
			...
			public void execute(EditingTarget target) {
				if (position != null) {
					...
					if (isDebugNavigation) {
						DebugFilePosition filePos = (DebugFilePosition) position
								.cast();
					...
					navigate(target, SourcePosition.create(
					...
					if (pos != null) {
						navigate(target,
								SourcePosition.create(pos.getRow(), 0), null);
					}
				}
			}
				...
				Scheduler.get().scheduleDeferred(new ScheduledCommand() {
					...
					public void execute() {
						if (navMethod == NavigationMethod.DebugStep) {
							...
							boolean highlight = navMethod == NavigationMethod.HighlightLine
									&& !uiPrefs_.highlightSelectedLine()
											.getValue();
							target.navigateToPosition(srcPosition, false,
									highlight);
						}
					}
				});
		...
		if (file != null && file.getLength() < 0) {
			...
			server_.stat(file.getPath(),
					new ServerRequestCallback<FileSystemItem>() {
						@Override
						...
						@Override
						public void onError(ServerError error) {
							// Couldn't stat the file? Proceed anyway. If the
							// file doesn't
							// exist, we'll let the downstream code be the one
							// to show the
							// error.
							action.execute(file);
						}
	...
	private void openFile(FileSystemItem file, TextFileType fileType) {
		openFile(file, fileType, new CommandWithArg<EditingTarget>() {
			@Override
			public void execute(EditingTarget arg) {
				...
				new ResultCallback<EditingTarget, ServerError>() {
					...
					public void onFailure(ServerError error) {
						...
						globalDisplay_.showMessage(GlobalDisplay.MSG_ERROR,
								"Error while opening file", message);
	...
	private void showFileTooLargeWarning(FileSystemItem file, long sizeLimit) {
		...
		globalDisplay_.showMessage(GlobalDisplay.MSG_WARNING,
				"Selected File Too Large", msg.toString());
	}

	private void confirmOpenLargeFile(FileSystemItem file,
			Operation openOperation, Operation cancelOperation) {
		StringBuilder msg = new StringBuilder();
		...
		globalDisplay_.showYesNoMessage(GlobalDisplay.MSG_WARNING,
				"Confirm Open", msg.toString(), openOperation, false); // 'No'
																		// is
																		// default
	}
	...
	private void openFileFromServer(final FileSystemItem file,
			final TextFileType fileType,
			final ResultCallback<EditingTarget, ServerError> resultCallback) {
		final Command dismissProgress = globalDisplay_
				.showProgress("Opening file...");

		server_.openDocument(file.getPath(), fileType.getTypeId(), uiPrefs_
				.defaultEncoding().getValue(),
	...
	private EditingTarget addTab(SourceDocument doc) {
		final EditingTarget target = editingTargetSource_.getEditingTarget(doc,
				fileContext_, new Provider<String>() {
					public String get() {
		...
		editors_.add(target);
		view_.addTab(widget, target.getIcon(), target.getName().getValue(),
				target.getTabTooltip(), // used as tooltip, if non-null
				true);
		fireDocTabsChanged();

		target.getName().addValueChangeHandler(
				new ValueChangeHandler<String>() {
					public void onValueChange(ValueChangeEvent<String> event) {
						view_.renameTab(widget, target.getIcon(),
								event.getValue(), target.getPath());
						fireDocTabsChanged();
					}
				});

		view_.setDirty(widget, target.dirtyState().getValue());
		target.dirtyState().addValueChangeHandler(
			...
			newDoc(FileTypeRegistry.R,
					new ResultCallback<EditingTarget, ServerError>() {
						public void onSuccess(EditingTarget arg) {
							((TextEditingTarget) arg).insertCode(
									event.getCode(), event.isBlock());
						}
					});
		}
	}
	...
	public void onTabClose(TabCloseEvent event) {
		...
		if (editors_.get(event.getTabIndex()).getId().equals(activeEditorId)) {
			...
			SourceNavigation srcNav = sourceNavigationHistory_
					.scanBack(new SourceNavigationHistory.Filter() {
						public boolean includeEntry(SourceNavigation navigation) {
							return !navigation.getDocumentId().equals(
									activeEditorId);
						}
					});
	...
	public void onTabReorder(TabReorderEvent event) {
		...
		if (event.getOldPos() < 0 || event.getOldPos() >= tabOrder_.size()
				|| event.getNewPos() < 0
				|| event.getNewPos() >= tabOrder_.size()) {
			return;
		}
	...
	public void onSelection(SelectionEvent<Integer> event) {
		...
		String paths = "";
		for (EditingTarget s : editors_) {
			paths = paths + ": " + s.getPath();
		}
	...
	private void manageCommands() {
		...
		HashSet<AppCommand> newCommands = activeEditor_ != null ? activeEditor_
				.getSupportedCommands() : new HashSet<AppCommand>();

		HashSet<AppCommand> commandsToEnable = new HashSet<AppCommand>(
				newCommands);
		commandsToEnable.removeAll(activeCommands_);

		HashSet<AppCommand> commandsToDisable = new HashSet<AppCommand>(
				activeCommands_);
	...
	private void manageVcsCommands() {
		...
		if (vcsCommandsEnabled) {
			String name = FileSystemItem.getNameFromPath(activeEditor_
					.getPath());
			commands_.vcsFileDiff().setMenuLabel("_Diff \"" + name + "\"");
			commands_.vcsFileLog().setMenuLabel("_Log of \"" + name + "\"");
			commands_.vcsFileRevert().setMenuLabel(
					"_Revert \"" + name + "\"...");
		}
	...
	private void manageRSConnectCommands() {
		boolean shinyCommandsAvailable = SessionUtils.showPublishUi(session_,
				uiPrefs_)
				&& (activeEditor_ != null)
				&& (activeEditor_.getPath() != null)
	...
	private void manageRMarkdownCommands() {
		boolean rmdCommandsAvailable = session_.getSessionInfo()
				.getRMarkdownPackageAvailable()
				&& (activeEditor_ != null)
	...
	private void pasteFileContentsAtCursor(final String path,
		...
		if (activeEditor_ != null && activeEditor_ instanceof TextEditingTarget) {
			...
			server_.getFileContents(path, encoding,
					new ServerRequestCallback<String>() {
						@Override
	...
	private void editFile(final String path) {
		server_.ensureFileExists(path, new ServerRequestCallback<Boolean>() {
			@Override
	...
	public void onSourceNavigateForward() {
		...
		if (navigation != null)
			attemptSourceNavigation(navigation,
					commands_.sourceNavigateForward());
	}
	...
	private void attemptSourceNavigation(final SourceNavigation navigation,
		...
		// check for code browser navigation
			...
			activateCodeBrowser(new SourceNavigationResultCallback<CodeBrowserEditingTarget>(
					navigation.getPosition(), retryCommand));
		}

		// check for file path navigation
		else if ((navigation.getPath() != null)
				&& !navigation.getPath().startsWith(DataItem.URI_PREFIX)) {
			FileSystemItem file = FileSystemItem.createFile(navigation
					.getPath());
			...
			public void onSuccess(CodeBrowserEditingTarget target) {
				...
				if (event.getDebugPosition() != null) {
					highlightDebugBrowserPosition(target,
							event.getDebugPosition(), event.getExecuting());
				}
			}
		});
	}
	...
	private void highlightDebugBrowserPosition(CodeBrowserEditingTarget target,
			DebugFilePosition pos, boolean executing) {
		target.highlightDebugLocation(
				SourcePosition.create(pos.getLine(), pos.getColumn() - 1),
				SourcePosition.create(pos.getEndLine(), pos.getEndColumn() + 1),
				executing);
	}
	...
	private void activateCodeBrowser(
		...
		if (codeBrowserTabIndex >= 0) {
			...
			// callback
			callback.onSuccess((CodeBrowserEditingTarget) editors_
					.get(codeBrowserTabIndex));
	...
	private final SourceNavigationHistory sourceNavigationHistory_ = new SourceNavigationHistory(
			30);
