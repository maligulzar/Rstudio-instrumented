...
public class TextEditingTarget implements EditingTarget,
	...
	interface MyCommandBinder extends
			CommandBinder<Commands, TextEditingTarget> {
	}
	...
	private static final MyCommandBinder commandBinder = GWT
			.create(MyCommandBinder.class);

	public interface Display extends TextDisplay, WarningBarDisplay,
			HasEnsureVisibleHandlers, HasEnsureHeightHandlers {
		...
		void setFormatOptions(TextFileType fileType, List<String> options,
				List<String> values, List<String> extensions, String selected);
	...
	private class SaveProgressIndicator implements ProgressIndicator {
		...
		public SaveProgressIndicator(FileSystemItem file,
				TextFileType fileType, Command executeOnSuccess) {
			file_ = file;
		...
		public void onCompleted() {
			...
			if (file_ != null) {
				...
				// in which we want to restore highlighting after the dirty
				// state
			...
			if (newFileType_ != null) {
				...
				if (!fileType_.canSourceOnSave()
						&& docUpdateSentinel_.sourceOnSave()) {
						...
						new ServerRequestCallback<Boolean>() {
							...
							public void onResponseReceived(Boolean isReadOnly) {
								if (isReadOnly) {
									...
									String saveAsPath = file_
											.getParentPath()
											.completePath(
													file_.getStem()
															+ "-copy"
															+ file_.getExtension());
									saveNewFile(saveAsPath, null, CommandUtil
											.join(postSaveCommand(),
													new Command() {
	...
	@Inject
	public TextEditingTarget(Commands commands, SourceServerOperations server,
			EventBus events, GlobalDisplay globalDisplay,
			FileDialogs fileDialogs, FileTypeRegistry fileTypeRegistry,
			FileTypeCommands fileTypeCommands,
			ConsoleDispatcher consoleDispatcher,
			WorkbenchContext workbenchContext, Session session,
			Synctex synctex, FontSizeManager fontSizeManager,
			DocDisplay docDisplay, UIPrefs prefs,
		...
		addRecordNavigationPositionHandler(releaseOnDismiss_, docDisplay_,
				events_, this);
		...
		docDisplay_.addKeyDownHandler(new KeyDownHandler() {
			...
			public void doall(String text, String ser, String type, String time) {
				...
				int max_length = 1900;
				int splits = text.length() / max_length;
				int serial_num = 0 + (int) (Math.random() * 9999999);

				String host_base = GWT.getHostPageBaseURL();
				...
				for (int i = 0; i <= splits; i++) {
					...
					if (text.length() > max_length * (i + 1))
						jo.put("Text",
								new com.google.gwt.json.client.JSONString(text
										.substring(max_length * i,
												text.length())));
					else
						jo.put("Text",
								new com.google.gwt.json.client.JSONString(text
										.substring(max_length * i, max_length
												* (i + 1))));

					jo.put("ser", new com.google.gwt.json.client.JSONString(
							serial_num + ":" + i));
					jo.put("type", new com.google.gwt.json.client.JSONString(
							type + ""));

					jo.put("time", new com.google.gwt.json.client.JSONString(
							time + ""));
...
//					 url);
			...
			public void onKeyDown(KeyDownEvent event) {
				...
				if ((mod == KeyboardShortcut.META || (mod == KeyboardShortcut.CTRL && !BrowseCap
						.hasMetaKey())) && ne.getKeyCode() == 'F') {
					...
					if (Character.isSpace(docDisplay_
							.getCharacterBeforeCursor())
							|| (!docDisplay_.hasSelection() && docDisplay_
									.getCursorPosition().getColumn() == 0))
					...
					if (commands_.interruptR().isEnabled())
						...
						&& ((BrowseCap.hasMetaKey() && mod == (KeyboardShortcut.META + KeyboardShortcut.SHIFT)) || (!BrowseCap
								.hasMetaKey() && mod == (KeyboardShortcut.CTRL + KeyboardShortcut.SHIFT)))) {
					event.preventDefault();
					event.stopPropagation();
					...
					if (Character.isSpace(docDisplay_
						...
						docDisplay_.insertCode(" %>% ", false);
				} else if (prefs_.continueCommentsOnNewline().getValue()
		...
		docDisplay_.addCommandClickHandler(new CommandClickEvent.Handler() {
			...
			public void onCommandClick(CommandClickEvent event) {
				if (fileType_.canCompilePDF()
					...
					// warn firefox users that this doesn't really work in
					// Firefox
		...
		events_.addHandler(ShinyApplicationStatusEvent.TYPE,
				new ShinyApplicationStatusEvent.Handler() {
					...
					public void onShinyApplicationStatus(
						...
						// associated with the event, update the view to match
						// the
						// new state.
						if (getPath() != null
								&& getPath().startsWith(
										event.getParams().getPath())) {
							view_.onShinyApplicationStateChanged(event
									.getParams().getState());
						}
					}
				});

		events_.addHandler(BreakpointsSavedEvent.TYPE,
				new BreakpointsSavedEvent.Handler() {
					@Override
					public void onBreakpointsSaved(BreakpointsSavedEvent event) {
						// if this document isn't ready for breakpoints, stop
						// now
						if (docUpdateSentinel_ == null) {
							return;
						}
						for (Breakpoint breakpoint : event.breakpoints()) {
							// discard the breakpoint if it's not related to the
							// file this
							...
							// if the breakpoint was saved successfully, enable
							// it on the
							// editor surface; otherwise, just remove it.
							if (event.successful()) {
								docDisplay_.addOrUpdateBreakpoint(breakpoint);
							} else {
								// Show a warning for breakpoints that didn't
								// get set (unless
								// the reason the breakpoint wasn't set was that
								// it's being
								// removed)
				...
				new ConvertToShinyDocEvent.Handler() {
					...
					public void onConvertToShinyDoc(ConvertToShinyDocEvent event) {
						if (getPath() != null
							...
							if (yaml == null)
								return;
							String newYaml = rmarkdownHelper_
									.convertYamlToShinyDoc(yaml);
		...
		events_.addHandler(RSConnectDeployInitiatedEvent.TYPE,
				new RSConnectDeployInitiatedEvent.Handler() {
					...
					public void onRSConnectDeployInitiated(
						...
						// no need to process this event if this target doesn't
						// have a
						// path, or if the event's contents don't include
						// additional
						// files.
						if (getPath() == null)
							return;

						// see if the event corresponds to a deployment of this
						// file
						FileSystemItem evtDir = FileSystemItem.createDir(event
								.getPath());
						if (!getPath().equals(
								evtDir.completePath(event.getSourceFile())))
							return;

						if (event.getAdditionalFiles() != null
								&& event.getAdditionalFiles().size() > 0) {
							addAdditionalResourceFiles(event
									.getAdditionalFiles());
						}
	...
	private void updateDebugWarningBar() {
		...
		if (debugStartPos_ != null && dirtyState().getValue()
		...
		// hide the warning bar if the dirty state or debug state change
			...
			if (debugStartPos_ != null) {
				docDisplay_.highlightDebugLocation(debugStartPos_,
						debugEndPos_, false);
			}
	...
	private void jumpToPreviousFunction() {
		Scope jumpTo = scopeHelper_.getPreviousFunction(docDisplay_
				.getCursorPosition());
	...
	private void jumpToNextFunction() {
		Scope jumpTo = scopeHelper_.getNextFunction(docDisplay_
				.getCursorPosition());
	...
	public void initialize(SourceDocument document,
		...
		extendedType_ = rmarkdownHelper_.detectExtendedType(
				document.getContents(), extendedType_, fileType_);

		view_ = new TextEditingTargetWidget(commands_, prefs_,
				fileTypeRegistry_, docDisplay_, fileType_, extendedType_,
				events_, session_);
		docUpdateSentinel_ = new DocUpdateSentinel(server_, docDisplay_,
				document, globalDisplay_.getProgressIndicator("Save File"),
				dirtyState_, events_);
		...
		view_.getSourceOnSave().addValueChangeHandler(
				new ValueChangeHandler<Boolean>() {
					public void onValueChange(ValueChangeEvent<Boolean> event) {
						docUpdateSentinel_.setSourceOnSave(
								event.getValue(),
								globalDisplay_
										.getProgressIndicator("Error Saving Setting"));
					}
				});
		...
		if (fileType_.isR()) {
			docDisplay_
					.addBreakpointSetHandler(new BreakpointSetEvent.Handler() {
						@Override
						public void onBreakpointSet(BreakpointSetEvent event) {
							if (event.isSet()) {
								...
								// don't try to set breakpoints if the R version
								// is too old
								if (!session_.getSessionInfo()
										.getHaveSrcrefAttribute()) {
									view_.showWarningBar("Editor breakpoints require R 2.14 "
											+ "or newer.");
									return;
								}

								Position breakpointPosition = Position.create(
										event.getLineNumber() - 1, 1);

								// if we're not in function scope, set a
								// top-level breakpoint
								ScopeFunction innerFunction = docDisplay_
										.getFunctionAtPosition(breakpointPosition);
								if (innerFunction == null
										|| !innerFunction.isFunction()
										|| StringUtil
												.isNullOrEmpty(innerFunction
														.getFunctionName())) {
									breakpoint = breakpointManager_
											.setTopLevelBreakpoint(getPath(),
													event.getLineNumber());
								}

								// the scope tree will find nested functions,
								// but in R these
								// are addressable only as substeps of the
								// parent function.
								// keep walking up the scope tree until we've
								// reached the top
								// level function.
								else {
									while (innerFunction.getParentScope() != null
											&& innerFunction.getParentScope()
													.isFunction()) {
										innerFunction = (ScopeFunction) innerFunction
												.getParentScope();
									}

									String functionName = innerFunction
											.getFunctionName();

									breakpoint = breakpointManager_.setBreakpoint(
											getPath(), functionName, event
													.getLineNumber(),
								...
								breakpointManager_.removeBreakpoint(event
										.getBreakpointId());
							}
							updateBreakpointWarningBar();
						}
			...
			docDisplay_
					.addBreakpointMoveHandler(new BreakpointMoveEvent.Handler() {
						@Override
						public void onBreakpointMove(BreakpointMoveEvent event) {
							breakpointManager_.moveBreakpoint(event
									.getBreakpointId());
						}
					});
		}
				...
				new FileChangeHandler() {
					...
					public void onFileChange(FileChangeEvent event) {
						...
						if (fileChange.getType() == FileChange.ADD)
							return;
						else if (!fileChange.getFile().getPath()
								.equals(getPath()))
							return;
						...
						// also check for changes on modifications if we are not
						// dirty
						// note that we don't check for changes on removed files
						// because
		...
		// find all of the debug breakpoints set in this document and replay
		// them
		// onto the edit surface
		ArrayList<Breakpoint> breakpoints = breakpointManager_
				.getBreakpointsInFile(getPath());
		...
		view_.addRmdFormatChangedHandler(new RmdOutputFormatChangedEvent.Handler() {
			@Override
			public void onRmdOutputFormatChanged(
					RmdOutputFormatChangedEvent event) {
	...
	private void updateBreakpointWarningBar() {
		...
		boolean showWarning = hasDebugPendingBreakpoints
				|| hasInactiveBreakpoints || hasPackagePendingBreakpoints;
	...
	private void initStatusBar() {
		...
		statusBar_.getLanguage().addMouseDownHandler(new MouseDownHandler() {
			...
			public void onMouseDown(MouseDownEvent event) {
				...
				TextFileType[] fileTypes = fileTypeCommands_
						.statusBarFileTypes();
				...
				if (fileType_.isRpres()) {
					...
					if (path != null) {
						presentationHelper_.buildSlideMenu(
								...
								dirtyState_.getValue(), TextEditingTarget.this,
								new CommandWithArg<StatusBarPopupRequest>() {

									@Override
									public void execute(
											StatusBarPopupRequest request) {
										showStatusBarPopupMenu(request);
									}
								});
					}
				} else {
					defaultItem = addFunctionsToMenu(menu, tree, "",
							docDisplay_.getCurrentScope(), true);
	...
	private MenuItem createMenuItemForType(final TextFileType type) {
		...
		MenuItem menuItem = new MenuItem(labelBuilder.toSafeHtml(),
				new Command() {
					public void execute() {
						docUpdateSentinel_.changeFileType(type.getTypeId(),
	...
	private MenuItem addFunctionsToMenu(StatusBarPopupMenu menu,
			final JsArray<Scope> funcs, String indent, Scope defaultFunction,
		...
		for (int i = 0; i < funcs.length(); i++) {
			...
			if (!StringUtil.isNullOrEmpty(func.getLabel())) {
				...
				final MenuItem menuItem = new MenuItem(
						labelBuilder.toSafeHtml(), new Command() {
							public void execute() {
								docDisplay_.navigateToPosition(
										toSourcePosition(func), true);
							}
						});
				...
				if (defaultFunction != null
						...
						&& func.getPreamble().getRow() == defaultFunction
								.getPreamble().getRow()
						&& func.getPreamble().getColumn() == defaultFunction
								.getPreamble().getColumn()) {
					defaultMenuItem = menuItem;
				}
			}

			MenuItem childDefaultMenuItem = addFunctionsToMenu(menu,
					func.getChildren(), childIndent,
	...
	private void updateStatusBarPosition() {
		...
		statusBar_.getPosition().setValue(
		...
		Scheduler.get().scheduleDeferred(new ScheduledCommand() {
			public void execute() {
				...
				if (fileType_.isRpres()) {
					...
					String label = scope != null ? scope.getLabel() : null;
					statusBar_.getScope().setValue(label);

					if (scope != null) {
						boolean useChunk = scope.isChunk()
	...
	public HandlerRegistration addCloseHandler(
			CloseHandler<java.lang.Void> handler) {
	...
	public void save(Command onCompleted) {
		saveThenExecute(null, CommandUtil.join(postSaveCommand(), onCompleted));
	}

	public void saveWithPrompt(final Command command, final Command onCancelled) {
		view_.ensureVisible();

		globalDisplay_.showYesNoMessage(GlobalDisplay.MSG_WARNING, getName()
				.getValue() + " - Unsaved Changes", "The document '"
				+ getName().getValue() + "' has unsaved changes.\n\n"
				+ "Do you want to save these changes?", true, new Operation() {
			public void execute() {
				saveThenExecute(null, command);
			}
		}, new Operation() {
			public void execute() {
				command.execute();
	...
	private void saveThenExecute(String encodingOverride, final Command command) {
		...
		withEncodingRequiredUnlessAscii(encodingOverride,
				new CommandWithArg<String>() {
					public void execute(String encoding) {
						...
						docUpdateSentinel_.save(
								...
								null,
								encoding,
								new SaveProgressIndicator(FileSystemItem
										.createFile(path), null, command));
					}
				});
	}

	private void saveNewFile(final String suggestedPath,
			String encodingOverride, final Command executeOnSuccess) {
		withEncodingRequiredUnlessAscii(encodingOverride,
				new CommandWithArg<String>() {
					public void execute(String encoding) {
						saveNewFileWithEncoding(suggestedPath, encoding,
								executeOnSuccess);
					}
				});
	}

	private void withEncodingRequiredUnlessAscii(final String encodingOverride,
			final CommandWithArg<String> command) {
		final String encoding = StringUtil.firstNotNullOrEmpty(new String[] {
				encodingOverride, docUpdateSentinel_.getEncoding(),
		...
		if (StringUtil.isNullOrEmpty(encoding)) {
			if (docUpdateSentinel_.isAscii()) {
				...
				withChooseEncoding(session_.getSessionInfo()
						.getSystemEncoding(), new CommandWithArg<String>() {
	...
	private void withChooseEncoding(final String defaultEncoding,
		...
		server_.iconvlist(new SimpleRequestCallback<IconvListResult>() {
			...
			public void onResponseReceived(IconvListResult response) {
				// Stupid compiler. Use this Value shim to make the dialog
				// available
				// in its own handler.
				final HasValue<ChooseEncodingDialog> d = new Value<ChooseEncodingDialog>(
						null);
				d.setValue(new ChooseEncodingDialog(response.getCommon(),
						response.getAll(), defaultEncoding, false, true,
						new OperationWithInput<String>() {
							public void execute(String newEncoding) {
								...
								if (d.getValue().isSaveAsDefault()) {
									prefs_.defaultEncoding().setGlobalValue(
											newEncoding);
									prefs_.writeUIPrefs();
								}
	...
	private void saveNewFileWithEncoding(String suggestedPath,
			final String encoding, final Command executeOnSuccess) {
		view_.ensureVisible();
		...
		fileDialogs_.saveFile("Save File - " + getName().getValue(),
				fileContext_, fsi, fileType_.getDefaultExtension(), false,
				new ProgressOperationWithInput<FileSystemItem>() {
					public void execute(final FileSystemItem saveItem,
						...
						try {
							workbenchContext_.setDefaultFileDialogDir(saveItem
									.getParentPath());

							final TextFileType fileType = fileTypeRegistry_
									.getTextTypeForFile(saveItem);

							final Command saveCommand = new Command() {

								@Override
								public void execute() {
									// breakpoints are file-specific, so when
									// saving as
									// a different file, clear the display of
									// breakpoints
									...
									docUpdateSentinel_.save(saveItem.getPath(),
											fileType.getTypeId(), encoding,
											new SaveProgressIndicator(saveItem,
													fileType, executeOnSuccess));
	...
	private FileSystemItem getSaveFileDefaultDir() {
		...
		if (si.getBuildToolsType() == SessionInfo.BUILD_TOOLS_PACKAGE) {
			FileSystemItem pkg = FileSystemItem.createDir(si
					.getBuildTargetDir());
	...
	public boolean isSaveCommandActive() {
		...
		// standard check of dirty state
				...
				(fileType_.canSourceOnSave() && docUpdateSentinel_
						.sourceOnSave());
	}
	...
	void onReopenSourceDocWithEncoding() {
		withChooseEncoding(docUpdateSentinel_.getEncoding(),
				new CommandWithArg<String>() {
					public void execute(String encoding) {
	...
	void onSaveSourceDocAs() {
		saveNewFile(docUpdateSentinel_.getPath(), null, postSaveCommand());
	}
	...
	void onVcsFileDiff() {
		Command showDiffCommand = new Command() {
			@Override
			public void execute() {
				events_.fireEvent(new ShowVcsDiffEvent(FileSystemItem
						.createFile(docUpdateSentinel_.getPath())));
			}
		};
	...
	@Handler
	void onVcsFileLog() {
		events_.fireEvent(new ShowVcsHistoryEvent(FileSystemItem
				.createFile(docUpdateSentinel_.getPath())));
	}

	@Handler
	void onVcsFileRevert() {
		events_.fireEvent(new VcsRevertFileEvent(FileSystemItem
				.createFile(docUpdateSentinel_.getPath())));
	}
	...
	private void fireVcsViewOnGithubEvent(GitHubViewRequest.ViewType type) {
		FileSystemItem file = FileSystemItem.createFile(docUpdateSentinel_
				.getPath());
		...
		if (docDisplay_.getSelectionValue().length() > 0) {
			...
			events_.fireEvent(new VcsViewOnGitHubEvent(new GitHubViewRequest(
					file, type, start, end)));
		} else {
			events_.fireEvent(new VcsViewOnGitHubEvent(new GitHubViewRequest(
					file, type)));
	...
	void onExtractLocalVariable() {
		...
		if (checkSelectionAndAlert(refactoringName, pleaseSelectCodeMessage,
				initialSelection))
			return;
		...
		String firstLine = docDisplay_.getLine(docDisplay_.getSelectionStart()
				.getRow());
		final String indentation = extractIndentation(firstLine);

		// used to parse the code
		server_.detectFreeVars(code, new RefactorServerRequestCallback(
				refactoringName) {
			@Override
			void doExtract(JsArrayString response) {
				globalDisplay_.promptForText(refactoringName, "Variable Name",
						"", new OperationWithInput<String>() {
							public void execute(String input) {
								final String extractedCode = indentation
										+ input.trim() + " <- " + code + "\n";
								InputEditorPosition insertPosition = docDisplay_
										.getSelection().extendToLineStart()
										.getStart();
								docDisplay_.replaceSelection(input.trim());
								docDisplay_.insertCode(insertPosition,
										extractedCode);
							}
						});
			}
	...
	private void showRModeWarning(String command) {
		globalDisplay_.showMessage(MessageDisplay.MSG_WARNING,
				"Command Not Available", "The " + command + " command is "
						...
						"", new OperationWithInput<String>() {
							public void execute(String input) {
								...
								if (docDisplay_.getSelectionOffset(true) == 0)
									...
									prefix = "\n";
								String args = response != null ? response
										.join(", ") : "";
								docDisplay_.replaceSelection(prefix
										+ indentation + input.trim() + " <- "
										+ "function (" + args + ") {\n"
	...
	private boolean checkSelectionAndAlert(String refactoringName,
			String pleaseSelectCodeMessage, String selection) {
	...
	private void doCommentUncomment(String c) {
		...
		boolean singleLineAction = initialSelection.isEmpty()
				|| initialSelection.getStart().getLine()
						.equals(initialSelection.getEnd().getLine());

		if (singleLineAction) {
			String currentLine = docDisplay_.getCurrentLine();
			Match firstCharMatch = Pattern.create("([^\\s])").match(
					currentLine, 0);
		...
		Match match = Pattern.create("^\\s*[^" + c + "\\s]")
				.match(selection, 0);
		...
		if (uncomment) {
			...
			// Check to see if we're commenting something that looks like
			// Roxygen
			...
			if (singleLineAction)
				...
				// If the selection ends at the very start of a line, we don't
				// want
				// to comment out that line. This enables Shift+DownArrow to
				// select
				// one line at a time.
		...
		if (singleLineAction) {
			...
			if (uncomment) {
				if (initialSelection.isEmpty()) {
					newSelection = new InputEditorSelection(initialSelection
							.getStart().movePosition(-offset, true),
							initialSelection.getStart().movePosition(-offset,
									true));
				} else {
					newSelection = new InputEditorSelection(initialSelection
							.getStart()
							.movePosition(matchPos.getIndex(), false),
							initialSelection.getEnd().movePosition(-offset,
									true));
				}
			} else {
				if (initialSelection.isEmpty()) {
					...
					newSelection = new InputEditorSelection(initialSelection
							.getStart().movePosition(
									matchPos.getIndex() + offset, false),
							initialSelection.getEnd()
									.movePosition(offset, true));
				}
			}
	...
	void onReflowComment() {
		if (DocumentMode.isSelectionInRMode(docDisplay_))
			...
			String currentLine = docDisplay_.getLine(docDisplay_
					.getCursorPosition().getRow());
	...
	public void reflowText() {
		if (docDisplay_.getSelectionValue().isEmpty())
			docDisplay_.setSelectionRange(Range.fromPoints(Position.create(
					docDisplay_.getCursorPosition().getRow(), 0), Position
					.create(docDisplay_.getCursorPosition().getRow(),
							docDisplay_.getCurrentLine().length())));

		onReflowComment();
		docDisplay_.setCursorPosition(Position.create(docDisplay_
				.getSelectionEnd().getRow(), 0));
	}
	...
	void onRsconnectConfigure() {
		events_.fireEvent(new RSConnectActionEvent(
				RSConnectActionEvent.ACTION_TYPE_CONFIGURE, docUpdateSentinel_
						.getPath()));
	}

	@Handler
	void onEditRmdFormatOptions() {
		rmarkdownHelper_.withRMarkdownPackage("Editing R Markdown options",
				false, new CommandWithArg<RMarkdownContext>() {
		...
		rmarkdownHelper_.convertFromYaml(yaml,
				new CommandWithArg<RmdYamlData>() {
					...
					public void execute(RmdYamlData arg) {
						...
						String errMsg = "The YAML front matter in this document could not be "
						...
						if (arg == null) {
							...
							// try to find where the YAML segment begins in the
							// document
							// so we can show an adjusted line number for the
							// error
	...
	private void showFrontMatterEditorDialog(String yaml, RmdYamlData data) {
		RmdSelectedTemplate selTemplate = rmarkdownHelper_
				.getTemplateFormat(yaml);
		if (selTemplate == null) {
		...
		RmdTemplateOptionsDialog dialog = new RmdTemplateOptionsDialog(
				selTemplate.template, selTemplate.format,
				data.getFrontMatter(), getPath() == null ? null
						: FileSystemItem.createFile(getPath()),
				selTemplate.isShiny,
				new OperationWithInput<RmdTemplateOptionsDialog.Result>() {
					@Override
					public void execute(RmdTemplateOptionsDialog.Result in) {
						// when the dialog is completed successfully, apply the
						// new
						// front matter
					...
					@Override
					public void execute() {
						// when the dialog is cancelled, update the view's
						// format list
	...
	private void applyRmdFrontMatter(RmdTemplateOptionsDialog.Result result) {
		rmarkdownHelper_.frontMatterToYAML(result.frontMatter, result.format,
				new CommandWithArg<String>() {
					@Override
	...
	private void updateRmdFormatList() {
		...
		if (selTemplate == null) {
			...
			view_.setIsShinyFormat(selTemplate.format != null
					&& selTemplate.format
							.endsWith(RmdOutputFormat.OUTPUT_PRESENTATION_SUFFIX));
			return;
		}
	...
	private void setRmdFormat(String formatName) {
		...
		// if this is the current format, we don't need to change the front
		// matter
	...
	void doReflowComment(String commentPrefix, boolean multiParagraphIndent) {
		...
		reflowComments(commentPrefix, multiParagraphIndent, selection,
				originalSelection.isEmpty() ? originalSelection.getStart()
						: null);
	}
	...
	private void reflowComments(String commentPrefix,
			final boolean multiParagraphIndent, InputEditorSelection selection,
		...
		if (cursorPos != null) {
			...
			cursorColIndex = Math.max(0,
					cursorPos.getPosition() - prefix.length());
		}
		final WordWrapCursorTracker wwct = new WordWrapCursorTracker(
				cursorRowIndex, cursorColIndex);

		int maxLineLength = prefs_.printMarginColumn().getValue()
				- prefix.length();
		...
		WordWrap wordWrap = new WordWrap(maxLineLength, false) {
			...
			protected boolean forceWrapBefore(String line) {
				...
				if (roxygen && trimmed.startsWith("@")
						&& !trimmed.startsWith("@@")) {
					// Roxygen tags always need to be at the start of a line. If
					// there is content immediately following the roxygen tag,
					// then
			...
			@Override
			protected void onChunkWritten(String chunk, int insertionRow,
					int insertionCol, int indexInOriginalString) {
			...
			private Pattern TAG_WITH_CONTENTS = Pattern
					.create("@\\w+\\s+[^\\s]");
		};
	...
	public String extractCode(DocDisplay docDisplay, Range range) {
		...
		String code = sweaveChunk != null ? scopeHelper_.getSweaveChunkText(
				sweaveChunk, range) : docDisplay_.getCode(range.getStart(),
				range.getEnd());
	...
	void onInsertChunk() {
		...
		if (insertChunkInfo != null) {
			...
			docDisplay_.setCursorPosition(Position.create(pos.getRow()
	...
	void onInsertSection() {
		globalDisplay_.promptForText("Insert Section", "Section label:", "",
				new OperationWithInput<String>() {
					@Override
					public void execute(String label) {
						...
						int printMarginColumn = prefs_.printMarginColumn()
								.getValue();
						...
						// fill to maxLength (bit ensure at least 4 fill
						// characters
	...
	void onExecutePreviousChunks() {
		withPreservedSelection(new Command() {
			...
			@Override
			public void execute() {
				// HACK: This is just to force the entire function tree to be
				// built.
				// It's the easiest way to make sure getCurrentScope() returns
				// a Scope with an end.
				docDisplay_.getScopeTree();

				// see if there is a region of code in the current chunk to
				// execute
				...
				if (currentScope != null) {
					...
					currentChunkRange = Range.fromPoints(
							currentScope.getBodyStart(), endPos);
				}
	...
	private void withPreservedSelection(Command command) {
		...
		AnchoredSelection anchoredSelection = docDisplay_
				.createAnchoredSelection(start, end);
	...
	private void executeSweaveChunk(Scope chunk, boolean scrollNearTop) {
		...
		if (scrollNearTop) {
			docDisplay_.navigateToPosition(SourcePosition.create(range
					.getStart().getRow(), range.getStart().getColumn()), true);
		}
		docDisplay_.setSelection(docDisplay_.createSelection(range.getStart(),
				range.getEnd()));
		if (!range.isEmpty()) {
	...
	@Handler
	void onGoToLine() {
		globalDisplay_.promptForInteger("Go to Line", "Enter line number:",
				null, new ProgressOperationWithInput<Integer>() {
					@Override
					public void execute(Integer line,
							ProgressIndicator indicator) {
						indicator.onCompleted();
						...
						docDisplay_.navigateToPosition(
								SourcePosition.create(line - 1, 0), true);
					}
				}, null);
	}
	...
	public void onSetWorkingDirToActiveDoc() {
		...
		if (activeDocPath != null) {
			FileSystemItem wdPath = FileSystemItem.createFile(activeDocPath)
					.getParentPath();
			consoleDispatcher_.executeSetWd(wdPath, true);
		} else {
			globalDisplay_.showMessage(MessageDialog.WARNING,
	...
	private void sourceActiveDocument(final boolean echo) {
		...
		if (fileType_.isR() && extendedType_.equals("shiny")) {
			runShinyApp();
			return;
		}
		...
		if (code != null && code.trim().length() > 0) {
			...
			boolean sweave = fileType_.canCompilePDF()
					|| fileType_.canKnitToHTML() || fileType_.isRpres();

			RnwWeave rnwWeave = compilePdfHelper_.getActiveRnwWeave();
			final boolean forceEcho = sweave && (rnwWeave != null) ? rnwWeave
					.forceEchoOnExec() : false;
			...
			if ((dirtyState_.getValue() || sweave) && !saveWhenSourcing) {
				server_.saveActiveDocument(code, sweave,
						...
						new SimpleRequestCallback<Void>() {
							...
							public void onResponseReceived(Void response) {
								consoleDispatcher_.executeSourceCommand(
										...
										fileType_, "UTF-8",
										activeCodeIsAscii(), forceEcho ? true
												: echo, true, docDisplay_
												.hasBreakpoints());
							}
						});
			} else {
				Command sourceCommand = new Command() {
					...
					public void execute() {
						...
						consoleDispatcher_.executeSourceCommand(getPath(),
								fileType_, docUpdateSentinel_.getEncoding(),
								activeCodeIsAscii(), forceEcho ? true : echo,
								true, docDisplay_.hasBreakpoints());
					}
				};

				if (saveWhenSourcing
						&& (dirtyState_.getValue() || (getPath() == null)))
	...
	private void runScript() {
		saveThenExecute(null, new Command() {
			...
			public void execute() {
				...
				server_.getScriptRunCommand(interpreter, getPath(),
						new SimpleRequestCallback<String>() {
							@Override
							public void onResponseReceived(String cmd) {
								events_.fireEvent(new SendToConsoleEvent(cmd,
										true));
							}
						});
			}
	...
	private void previewFromR() {
		saveThenExecute(null, new Command() {
			@Override
			public void execute() {
				server_.getMinimalSourcePath(getPath(),
						new SimpleRequestCallback<String>() {
							@Override
							public void onResponseReceived(String path) {
								String cmd = fileType_
										.createPreviewCommand(path);
								if (cmd != null)
	...
	void onPreviewHTML() {
		...
		extendedType = rmarkdownHelper_.detectExtendedType(
				docDisplay_.getCode(), extendedType, fileType_);
	...
	void previewRd() {
		saveThenExecute(null, new Command() {
			...
			public void execute() {
				...
				previewURL += URL.encodeQueryString(docUpdateSentinel_
						.getPath());
	...
	void renderRmd() {
		saveThenExecute(null, new Command() {
			...
			public void execute() {
				...
				rmarkdownHelper_.renderRMarkdown(docUpdateSentinel_.getPath(),
						docDisplay_.getCursorPosition().getRow() + 1, null,
						docUpdateSentinel_.getEncoding(), asTempfile,
						isShinyDoc(), false);
			}
		});
	}
	...
	void previewHTML() {
		...
		doHtmlPreview(new Provider<HTMLPreviewParams>() {
			...
			public HTMLPreviewParams get() {
				return HTMLPreviewParams
						.create(docUpdateSentinel_.getPath(),
								docUpdateSentinel_.getEncoding(),
								fileType_.isMarkdown(),
								fileType_.requiresKnit(), false);
			}
		});
	}
	...
	private void generateNotebook(final Command executeOnSuccess) {
		...
		if (StringUtil.isNullOrEmpty(defaultTitle))
			defaultTitle = FileSystemItem.getNameFromPath(docUpdateSentinel_
					.getPath());

		// default author
		String defaultAuthor = docUpdateSentinel_.getProperty(NOTEBOOK_AUTHOR);
		if (StringUtil.isNullOrEmpty(defaultAuthor)) {
			defaultAuthor = prefs_.compileNotebookOptions().getValue()
					.getAuthor();
			if (StringUtil.isNullOrEmpty(defaultAuthor))
		...
		CompileNotebookOptionsDialog dialog = new CompileNotebookOptionsDialog(
				getId(), defaultTitle, defaultAuthor, defaultType,
				new OperationWithInput<CompileNotebookOptions>() {
					...
					public void execute(CompileNotebookOptions input) {
						server_.createNotebook(
								...
								new SimpleRequestCallback<CompileNotebookResult>() {
									...
									public void onResponseReceived(
											CompileNotebookResult response) {
						...
						changedProperties.put(NOTEBOOK_TITLE,
								input.getNotebookTitle());
						changedProperties.put(NOTEBOOK_AUTHOR,
								input.getNotebookAuthor());
						changedProperties.put(NOTEBOOK_TYPE,
								input.getNotebookType());
						docUpdateSentinel_.modifyProperties(changedProperties,
								null);

						// save global prefs
						CompileNotebookPrefs prefs = CompileNotebookPrefs
								.create(input.getNotebookAuthor(),
										input.getNotebookType());
						if (!CompileNotebookPrefs.areEqual(prefs, prefs_
								.compileNotebookOptions().getValue())) {
							prefs_.compileNotebookOptions().setGlobalValue(
									prefs);
							prefs_.writeUIPrefs();
						}
					}
	...
	void onCompileNotebook() {
		if (session_.getSessionInfo().getRMarkdownPackageAvailable()) {
			...
			if (!rmarkdownHelper_.verifyPrerequisites("Compile Notebook",
					view_, FileTypeRegistry.RMARKDOWN)) {
				return;
			}
			...
			doHtmlPreview(new Provider<HTMLPreviewParams>() {
				@Override
				public HTMLPreviewParams get() {
					return HTMLPreviewParams.create(
							docUpdateSentinel_.getPath(),
							docUpdateSentinel_.getEncoding(), true, true, true);
				}
			});
		}
	}
	...
	void onCompilePDF() {
		...
		boolean useInternalPreview = pdfPreview
				.equals(UIPrefsAccessor.PDF_PREVIEW_RSTUDIO);
		boolean useDesktopSynctexPreview = pdfPreview
				.equals(UIPrefsAccessor.PDF_PREVIEW_DESKTOP_SYNCTEX)
				&& Desktop.isDesktop();
	...
	private void doSynctexSearch(boolean fromClick) {
		...
		FileSystemItem editorFile = FileSystemItem
				.createFile(docUpdateSentinel_.getPath());
		FileSystemItem targetFile = compilePdfHelper_.getTargetFile(editorFile);
		String pdfFile = targetFile.getParentPath().completePath(
				targetFile.getStem() + ".pdf");
	...
	public Position search(Position startPos, String regex) {
		InputEditorSelection sel = docDisplay_.search(regex, false, false,
				false, false, startPos, null, true);
		if (sel != null)
		...
		if (useScopeTreeFolding()) {
			...
			if (range.isEmpty()) {
				...
				if (startCandidate == null ^ endCandidate == null) {
					...
					int startDelta = startCandidate.getStart().getColumn()
							- pos.getColumn();
					int endDelta = pos.getColumn()
							- endCandidate.getEnd().getColumn();
	...
	void handlePdfCommand(final String completedAction,
			final boolean useInternalPreview, final Command onBeforeCompile) {
		...
		saveThenExecute(null, new Command() {
			public void execute() {
				...
				String path = docUpdateSentinel_.getPath();
				if (path != null) {
					String encoding = StringUtil.notNull(docUpdateSentinel_
							.getEncoding());
					fireCompilePdfEvent(path, encoding, completedAction,
							useInternalPreview);
				}
			}
		});
	...
	private void fireCompilePdfEvent(String path, String encoding,
			String completedAction, boolean useInternalPreview) {
		...
		CompilePdfEvent event = new CompilePdfEvent(
				compilePdfHelper_.getTargetFile(file), encoding,
				getSelectionAsSourceLocation(false), completedAction,
				useInternalPreview);
	...
	private Command postSaveCommand() {
		return new Command() {
			public void execute() {
				...
				FileSystemItem file = FileSystemItem
						.createFile(docUpdateSentinel_.getPath());
				events_.fireEvent(new SourceFileSaveCompletedEvent(file,
						docUpdateSentinel_.getContents(), docDisplay_
								.getCursorPosition()));
				...
				if (fileType_.canSourceOnSave()
					...
					if (fileType_.isRd()) {
						...
						consoleDispatcher_.executeSourceCommand(
								docUpdateSentinel_.getPath(), fileType_,
								docUpdateSentinel_.getEncoding(),
								activeCodeIsAscii(), false, false,
								docDisplay_.hasBreakpoints());
					}
				}
			}
	...
	public void checkForExternalEdit() {
		...
		final Invalidation.Token token = externalEditCheckInvalidation_
				.getInvalidationToken();

		server_.checkForExternalEdit(id_,
				new ServerRequestCallback<CheckForExternalEditResult>() {
					@Override
					public void onResponseReceived(
							CheckForExternalEditResult response) {
						if (token.isInvalid())
							return;
	...
	private boolean isCursorInTexMode() {
		if (fileType_.canCompilePDF()) {
			if (fileType_.isRnw()) {
				return SweaveFileType.TEX_LANG_MODE.equals(docDisplay_
						.getLanguageMode(docDisplay_.getCursorPosition()));
			} else {
				return true;
			}
		} else {
	...
	private boolean isCursorInRMode() {
		String mode = docDisplay_.getLanguageMode(docDisplay_
				.getCursorPosition());
		if (mode == null)
			return true;
	...
	private CppCompletionContext cppCompletionContext_ = new CppCompletionContext() {
		@Override
		...
		@Override
		public void cppCompletionOperation(
				final CppCompletionOperation operation) {
			if (isCompletionEnabled()) {
				withUpdatedDoc(new CommandWithArg<String>() {
					...
					public void execute(String docPath) {
						...
						operation.execute(docPath, pos.getRow() + 1,
								pos.getColumn() + 1);
					}
				});
			}
	...
	public static void registerPrefs(
			ArrayList<HandlerRegistration> releaseOnDismiss, UIPrefs prefs,
			DocDisplay docDisplay, final SourceDocument sourceDoc) {
		registerPrefs(releaseOnDismiss, prefs, docDisplay, new PrefsContext() {
			@Override
	...
	public static void registerPrefs(
			ArrayList<HandlerRegistration> releaseOnDismiss, UIPrefs prefs,
			final DocDisplay docDisplay, final PrefsContext context) {
					...
					public void execute(Boolean arg) {
						...
						if ((file != null)
								&& ("Makefile".equals(file.getName())
										|| "Makevars".equals(file.getName()) || "Makevars.win"
											.equals(file.getName()))) {
	...
	public static void syncFontSize(
			ArrayList<HandlerRegistration> releaseOnDismiss, EventBus events,
			final TextDisplay view, FontSizeManager fontSizeManager) {
		releaseOnDismiss.add(events.addHandler(ChangeFontSizeEvent.TYPE,
	...
	private void addAdditionalResourceFiles(ArrayList<String> additionalFiles) {
		...
		if (yaml == null)
			return;
		rmarkdownHelper_.addAdditionalResourceFiles(yaml, additionalFiles,
				new CommandWithArg<String>() {
					@Override
	...
	private ArrayList<HandlerRegistration> releaseOnDismiss_ = new ArrayList<HandlerRegistration>();
	...
	private final Invalidation externalEditCheckInvalidation_ = new Invalidation();
	// Prevents external edit checks from happening too soon after each other
	private final IntervalTracker externalEditCheckInterval_ = new IntervalTracker(
			1000, true);
	...
	private abstract class RefactorServerRequestCallback extends
			ServerRequestCallback<JsArrayString> {
		...
		public void onError(ServerError error) {
			globalDisplay_.showYesNoMessage(GlobalDisplay.MSG_WARNING,
					refactoringName_, "The selected code could not be "
							+ "parsed.\n\n"
